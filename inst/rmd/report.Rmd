...
```{r checkDependencies, echo=params$showCode, message=FALSE, warning=FALSE}
...
DT::datatable(data.frame(
  min = c(min(defFuns$nArgs, na.rm = TRUE), min(defFuns$cycloComp, na.rm = TRUE)),
  max = c(max(defFuns$nArgs, na.rm = TRUE), max(defFuns$cycloComp, na.rm = TRUE)),
  mean = c(round(mean(defFuns$nArgs, na.rm = TRUE), 2), round(mean(defFuns$cycloComp, na.rm = TRUE), 2)),
  median = c(median(defFuns$nArgs, na.rm = TRUE), median(defFuns$cycloComp, na.rm = TRUE)),
  sd = c(round(sd(defFuns$nArgs, na.rm = TRUE), 2), round(sd(defFuns$cycloComp, na.rm = TRUE), 2)),
  row.names = c("Number of Arguments", "Cyclomatic Complexity")))
```
